import type { SiteConfig } from "@/types";

export const siteConfig: SiteConfig = {
  name: "Afip SDK",
  description:
    "Afip SDK es la biblioteca preferida entre los desarrolladores para integrar sus plataformas con AFIP.",
  url: "https://afipsdk.com",
  ogImage: "https://afipsdk.com/og.png",
  contact: "ivan@afipsdk.com",
  links: {
    discord: "https://discord.gg/A6TuHEyAZm",
    github: "https://github.com/afipsdk",
    docs: "https://docs.afipsdk.com",
    sdkPython: 'https://github.com/afipsdk/afip.py',
    sdkJs: 'https://github.com/afipsdk/afip.js',
    sdkPhp: 'https://github.com/afipsdk/afip.php',
    sdkRuby: 'https://github.com/afipsdk/afip.rb'
  }
};
